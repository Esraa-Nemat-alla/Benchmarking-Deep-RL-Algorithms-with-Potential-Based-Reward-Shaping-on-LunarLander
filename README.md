...
To make sure everyone on the team understands the codebase, here is a breakdown of the 4 main Python scripts and what you should edit in each one:
...
* **What it does:** The main training script. It connects an algorithm to the shaped environment, trains it, evaluates it periodically using an `EvalCallback`, and saves the final model to the `results/` folder.
...
* **What it does:** Instead of typing commands manually for every configuration, this script loops through all combinations (Algorithms $\times$ Rewards $\times$ Seeds) and runs `train.py` automatically as a separate process.
* **What to do here:** If you want to scale up to 1,000,000 timesteps or run 5 seeds for the final report, change the `TIMESTEPS` and `SEEDS` variables at the top of this file.
...
* `monitor.csv` - training episode returns/lengths
...
## 6. Design Choices & Proposal Mapping
